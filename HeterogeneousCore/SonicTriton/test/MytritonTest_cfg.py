...
models = {
   ...
   # "TritonIdentityProducer": ["ragged_io"],
    "PatPhotonDRNCorrectionProducer" : ["DRN"]
}
...
options.register("modules", "PatPhotonDRNCorrectionProducer", VarParsing.multiplicity.list, VarParsing.varType.string, "list of modules to run (choices: {})".format(', '.join(models)))
options.register("models","DRN", VarParsing.multiplicity.list, VarParsing.varType.string, "list of models (same length as modules, or just 1 entry if all modules use same model)")
...
if options.device != "auto":
    process.TritonService.fallback.useGPU = options.device=="cpu"
    ...
    setattr(process, module,
        Module(module,
            Client = cms.PSet(
                ...
                modelConfigPath = cms.FileInPath("/eos/home-b/bbapi/Energy_regression/CMSSW_13_3_3/src/RecoEgamma/EgammaTools/data/models/photonObjectBothEnsemble/config.pbtxt"),
